...
# TODOs: check routing with bytes
# TODOs: check routing with smaller copies in/out, explore memref.view??
...
# TODO: why in 32b? why not just stay with 8b?
...
if __name__ == "__main__":
    ...
    activationsIn = np.full(shape=(activationsInSize32b,), fill_value=1, dtype=np.int32)
    weightsIn = np.zeros(shape=(bn10_total_wts_size,), dtype=np.int8)
    for i in range(bn10_total_wts_size):
        if i < bn10_layer1_wts_size:
            ...
            weightsIn[i] = 7
    activationsOut = np.zeros(shape=(activationsOutSize32b,), dtype=np.int32)
